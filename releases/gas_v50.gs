  ...
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][0] === p.id) {
      sh.getRange(i+1,1,1,12).setValues([[
        ...
        p.stockLoc||0, p.stockShip||0, p.sharedStockWith||'', p.costPrice||0
      ]]);
      return ok({ saved: true });
    }
  }
...
function setup() {
  ...
  createSheetWithHeaders(ss, SH.PRODUCTS,  ['id','name','price','totalMinutes','stepTimesJson','stock','stockWarn','typesJson','stockLoc','stockShip','sharedStockWith','costPrice']);
...
function fixAllSheets() {
  ...
  fixHeader(ss,'products', ['id','name','price','totalMinutes','stepTimesJson','stock','stockWarn','typesJson','stockLoc','stockShip','sharedStockWith','costPrice']);
