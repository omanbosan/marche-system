...
> 版ごとの変更一覧（開発履歴）は [CHANGELOG.md](CHANGELOG.md) にまとめてある。
> このファイルは設計の背景・注意点・再発防止メモを日付順に積んだ引き継ぎ用。
...
### 2026-09-21: 読み書きが遅い問題の調査（現地マルシェで2人が同時に使うと遅い）と、実施済みの軽量化（v96に含む）
**実測・コード調査で分かったこと**
- **GASの最低応答時間**: 認証なしの何もしないリクエストでも、この端末から**約1.1〜1.3秒**（暖機時）、しばらく放置後は**2.4〜3.5秒**（コールドスタート）。つまり「通信1回＝最低約1.2秒」で、直列に何回も呼ぶ処理ほど遅い。マルシェの回線（テザリング等）ではさらに伸びる。※本番の`getAll`自体の所要時間は未計測（ログインしての実測が必要）
- **`getAll`のキャッシュが効きにくい**: 書き込みのたびに`invalidateCache()`で消す設計のため、2人が書き込み続けるとほぼ毎回キャッシュミス。ミス時はorders/items/steps/productsを**全期間の全行**読む（stepsは1商品につき6〜7行で増え続ける）。結果JSONが95KBを超えるとキャッシュ自体が保存されない
- **自動同期が二重**: `startClock()`内の20秒ごとと、初期化側の30秒ごと（`isInputting`判定つき）の2つのタイマーが両方動いており、端末1台あたり毎分3〜5回`getAll`を叩く。2台なら倍
- **注文登録が遅く感じる原因**: `saveOrder()`は`saveOrderFast`の応答を待ってから画面に注文を反映する（全画面ローディングも出る）。GAS側は約12回のシートAPI（履歴シート全件の重複チェック読み込み含む）。送信データが4000文字を超える注文（目安7点以上）は分割送信で**通信が複数回に直列化**（1回ごとに約1.2秒）。さらに在庫引き落としが商品ごとに`adjustStock`を別通信で呼び、GAS側は**全体で1つのScriptLock**（最大10秒待ち）＋productsシート全件読みなので、2人が同時に登録すると順番待ちになる
- **`handleGetHistory`がO(n²)**だった（履歴1件ごとにsales/itemsの全件をfilter）。データが増えるほど急に遅くなる

**実施済み（本コミット・未デプロイ）**
- `handleGetHistory`を索引（マップ）方式に変更（出力は変更前と完全一致することをモックデータ400件・4パターンで確認）。`lite=true`で工程・アイテムの全件読み込みを省く軽量版を追加（ホーム用）
- 新アクション`getDashData`（履歴lite＋経費＋固定費を1通信で返す）を追加。ホームは年ごとに3通信→1通信になった。GAS未更新のときは自動で従来の3通信（lite付き）にフォールバック

**未実施の提案（優先順）**
1. 自動同期の整理（重複タイマー削除・45〜60秒間隔・多重実行ガード）、送信データの軽量化（`stepIds`を送らずitemIdから導出して分割送信を回避）、`saveOrderFast`内の履歴全件読み込み削除
2. 注文登録の**楽観的UI**（押した瞬間に画面へ反映、裏で送信）＋localStorageの**送信待ちキュー（アウトボックス）**で自動再送（サーバー側は注文IDで冪等なので安全）。現地の電波不安定対策にもなる
3. 在庫引き落としを`saveOrderFast`と同じ通信で1回のロックにまとめる
4. 完了済みの古い注文をアーカイブシートへ退避する夜間トリガー（`getAll`の読み込み行数を一定に保つ）。分析タブの工程別平均が使うstepsの扱いに注意
5. 最終手段：バックエンドをFirebase/Supabase等のリアルタイムDBへ移行（書き込み0.1〜0.3秒・2台で即時同期・オフライン対応）、Sheetsは会計用に夜間エクスポート

### 2026-09-21: 読み書き高速化（v97）— 楽観的UI＋送信キュー・在庫の一括処理・送信量削減
前項の調査で挙げた提案1〜3を、機能面を検証したうえで実装した（提案4のアーカイブ・5のDB移行は未着手）。

**1. 自動同期の整理**
- `startClock()`内の20秒タイマーを**削除**。初期化側の30秒タイマー（入力中・モーダル表示中はスキップ）だけに一本化した。旧20秒タイマーは入力中でも走るため、受付入力の最中に`S.orders`/`S.products`がサーバー内容で丸ごと上書きされる原因でもあった
- `loadAll()`は実行中なら同じPromiseを返す（自動同期・手動同期・画面復帰が重なっても通信は1回）
- 画面が非表示のとき（スリープ・別アプリ）は同期しない。復帰時に`visibilitychange`で取り直す
- 検証：ヘッダー高さ・ピル配置は375/390/430pxで変更前と同一（横はみ出しなし）

**2. 送信キュー（アウトボックス）と楽観的UI** — `src/index.html`の「送信キュー」セクション
- `apiAsync()`の中身を**キューへの登録**に変更した。呼び出し側（工程タップ・保留・入金確認など20箇所以上）は**書き換え不要**
- 受付・完了・削除・注文編集も`await api(...)`をやめてキュー送信にした。画面はその場で更新される
- 1件ずつ**順番に**送る。これにより「受付がまだ送信待ちなのに完了を先に送ってしまう」事故が起きない（旧実装では`await`で順序が保証されていた。楽観的UIにするならキューの直列化が必須）
- `localStorage`（`mb_outbox_v1`）に保存するので、途中でブラウザを閉じても次回起動時に続きから送る
- ネットワーク障害は**何度でも**再送（2/5/10/20/40秒のバックオフ）。サーバーが`ok:false`を返した場合のみ4回で停止し、ヘッダーに⚠️を出してタップで手動再送。**データは捨てない**
- `Unauthorized`はキューを止めてログイン画面へ。再ログイン（`doLogin`）で自動再開
- ヘッダーに未送信バッジ（`#ob-pill`）。未送信が残ったままページを閉じようとすると確認ダイアログを出す
- `loadAll()`後に`obApplyPendingLocal()`で、まだ送っていない受付・完了・削除を画面に重ね直す。これをしないと自動同期のたびに「受付した注文が消える」「完了した注文が戻る」ように見える
- 完了がサーバーに拒否され続けた場合は`obOnFailed()`で一覧に戻す（2026-08-08のv83対策と同じ考え方を維持）

**3. 在庫の一括処理（GAS）** — `adjustStockBatch()`
- 注文登録・注文削除は商品ごとに`handleAdjustStock`を呼んでいたため、**ScriptLockの取得とproductsシート全件読み込みが商品の数だけ**繰り返されていた。2人同時操作時の待ち行列の主因
- `adjustStockBatch(ss, list, reason)`で**1回のロック・1回の読み込み**にまとめた。`sharedStockWith`の解決・0での打ち止め・タイプ別/単品の判定は従来と同じ
- `handleSaveOrderFast`が`data.serverStock===true`のときだけ在庫を引く。**新旧どちらの組み合わせでも二重に引かない／引き忘れない**ようにしてある：
  - 新フロント＋旧GAS → GASは`serverStock`を無視し`stockApplied`も返さない → フロントが従来どおり商品ごとの`adjustStock`にフォールバック
  - 旧フロント＋新GAS → `serverStock`が来ないのでGASは在庫を触らず、旧フロント自身の`adjustStock`が効く
  - 再送で同じ注文が2回届いた場合 → 冒頭の重複チェックで`stockApplied:true`を返すのでフォールバックが走らない
- 検証：旧実装（商品ごと）と新実装（一括）に同じ増減を与えて**最終在庫が7ケースすべて一致**（同一商品の複数点／タイプ別混在／現地・郵送混在／在庫連動/0での打ち止め／削除の戻し／存在しない商品）。`stock_log`は集約されるため行数は減る（`★NOT_FOUND★`の記録は残すようにした）

**4. 送信量の削減とステップIDの決定化**
- 工程IDを`アイテムID-工程番号`に統一（フロント・GAS双方が同じ規則で組み立てる）。`stepIds`を送る必要がなくなった
- `paid`/`onHold`/`step0Done`/`step0At`も常に同じ値なので送信から除外（GASが既定値で補完。旧フロントが送ってきた場合は従来どおり優先）
- 実測：**14点の注文で 2リクエスト・6,164文字 → 1リクエスト・3,355文字**。分割送信（1回ごとに約1.2秒の直列通信）が起きなくなった
- 副次効果として既存バグを解消：`handleAddItemToOrder`の応答が届かないとき、フロントは工程IDを`'s0'`等の仮IDにしていたためサーバーと不一致になり、その後の工程更新が無言で効かなくなっていた

**5. `handleSaveOrderFast`からhistory全件読み込みを削除**
- `recordSalesForOrder`に`skipDupCheck`引数を追加。`handleSaveOrderFast`は冒頭で「ordersに同じIDが無い＝新規」を確認済みで、historyの行はordersの行の後にしか作られないため、この経路では重複チェックが不要。historyシートは増え続けるので効果が大きい

**検証方法（今後も同じ手順を使うこと）**
- `window.fetch`をモックGAS（在庫・注文・工程の状態を持つ）に差し替えたテストページを生成し、ヘッドレスChromeで18シーンを実行してエラー0件を確認：ホーム6シーン＋書き込み12シーン（楽観的反映／圏外→復帰／送信順序／サーバー拒否→手動再送／再読み込みをまたいだ再送と冪等性／送信サイズ／削除と在庫復元／旧GASフォールバック／トークン切れ→再ログイン／送信待ちのまま完了／完了の拒否→一覧復帰）
- GAS側は旧版（`releases/gas_v96.gs`）と新版を同じモックシートに読み込ませ、出力・最終在庫の一致を比較する方式（`handleGetHistory`4パターン、在庫7ケース）
- スクリプトは`scratchpad`の`mkwrite*.ps1`／`mkstock.ps1`／`mkhdr.ps1`。**PowerShell 5.1はBOM無しUTF-8スクリプトを文字化けするので、実行前にBOM付きで保存し直すこと**
...
### 2026-09-21: 2人運用（受付→2値化→デザインの分担）向けの修正（v98・フロントのみ）
ユーザーから「1人目が受付登録→もう1人が2値化→1人目がデザイン、という分担で使う。受付登録も2人が交互に行う」との運用説明を受けて検証し、v97の作りで実際に困る3点を修正した。

1. **自動同期を30秒→15秒に戻した**
   - 相手の操作が自分の画面に出るまでの時間が、そのまま次の工程の待ち時間になる運用だった
   - v97で20秒タイマーを削除して30秒1本にしたが、従来は20秒＋30秒の2本で実質12秒間隔だったため、**待ち時間が倍以上に伸びていた**。15秒1本なら従来と同等で、リクエスト数は従来より少ない
2. **送信待ちの工程タップ・保留・入金確認を、同期後に描き直すようにした**（`obApplyPendingLocal`に`updateStep`/`updateItem`を追加）
   - キューが詰まっている間（前の注文が再送中など）に自動同期が走ると、**押したはずの工程が一瞬戻って見える**状態だった。v97では受付・完了・削除しか重ね直していなかった
3. **受付モーダルを開いたときに最新データを取り直すようにした**（`openNewOrder`で`loadAll`→`updateSuggestedOrderNum`）
   - 2人が交互に受付すると、相手が採番した番号を自分の端末が知らないまま**同じ受付番号を提案してしまう**（これはv97以前からの問題だが、同期間隔が延びて起きやすくなっていた）
   - `loadAll`は`renderPPick`を呼ばないので、選択中の商品は消えない。`T.numManuallyEdited`が立っていれば上書きしない

**検証**：モックGASを2台の端末に見立てて、受付→2値化→デザインの受け渡しで工程が取りこぼされないこと、送信待ち中に同期が走っても工程が戻らないこと、受付番号が重複しないことを確認（全19シーン・エラー0件）。

**残る制限（設計上どうにもならない点。相談が来たら説明すること）**
- 相手の操作が見えるまで最大15秒かかる。GASはポーリングしかできないため、即時反映にするにはリアルタイムDB（Firebase等）への移行が必要
- 在庫の残数表示も同期間隔ぶん遅れる。ただし**引き落とし自体はサーバー側のロックで正しく処理される**ので、数が壊れることはない（最後の1点を2人が同時に受け付けると、画面上は通っても在庫は0で止まる）

### 2026-09-21: 入力モーダルが勝手に閉じる不具合の修正・ホーム画面の並び替え（v99・フロントのみ）
1. **入力欄から左へドラッグするとモーダルが消えて入力が失われる不具合を修正**（最優先）
   - `.overlay`の「背景クリックで閉じる」が`click`の`target`だけを見ていたため、**入力欄で押し始めて背景で指/マウスを離す**（文字のドラッグ選択で外に出た動き）と、clickの対象が背景になり閉じてしまっていた
   - `pointerdown`の対象を記録し、**押し始めも背景だったときだけ閉じる**ようにした（`attachOverlayDismiss()`）。動的生成する履歴編集モーダルも同じ関数を使う
   - **今後オーバーレイを追加するときは必ず`attachOverlayDismiss(ov)`を使うこと。`ov.addEventListener('click', ...)`を直接書かない**
2. **累計グラフの系列名を期間に合わせた**：月次で「今期」と出ていて分かりにくいとの指摘。月次は「今月／◯月」、年次は「今年／◯年」（`curLbl`）。見出しも同じ語に揃えた
3. **ホーム画面の並びを変更**：挨拶ブロック（「おまんぼさんのショップ島」「今日も楽しくお店を育てよう！」の3行）を削除。並びは **レベル → クエスト＋今日のミッション → 期間切替 → KPI → グラフ → 内訳** に変更（毎日クエストから始められるように最上段へ）
...
### 2026-09-21: スマホでの初回表示が遅い問題の修正（v102）
「スマホで見ると読み込みに時間がかかる」との報告を受けて実測したところ、**原因はアプリ本体ではなく日本語Webフォント**だった。

**実測値**
- アプリ本体（index.html）: gzip後 **113KB**
- Google Fonts の CSS: **570KB**（@font-faceが614個）。`rel="stylesheet"`なので**これを読み終わるまで画面が出ない**
- フォント本体（分割ファイル366個の合計）: **7.8MB**。実際に落ちるのは表示する文字の範囲だけだが、5ウェイト（Zen Kaku 700/900＋Noto 400/500/700）ぶん重複して落ちる

**対処**
- フォントCSSを`media="print"` + `onload="this.media='all'"`で読み込む**描画をせき止めない方式**に変更（`<noscript>`のフォールバックも設置）。文字はまず端末標準の日本語フォントで出て、あとからWebフォントに差し替わる
- `<link rel="preconnect">`を`fonts.googleapis.com`と`fonts.gstatic.com`に追加
- **Noto Sans JP の 500 を外した**（CSS内で`font-weight:500`の直接指定は0件。600指定15件が500に寄っていただけで、700に寄っても見た目はほぼ変わらない）。これでフォントCSSが149KB→119KBに減った
- フォールバックのfont-familyを`sans-serif`から`-apple-system,BlinkMacSystemFont,'Hiragino Sans','Hiragino Kaku Gothic ProN','Yu Gothic',Meiryo,sans-serif`に変更（42箇所）。Webフォントが来るまでの表示が端末標準の日本語ゴシックになる

**効果（ヘッドレスChrome・390px幅で実測）**
| | 変更前(v101) | 変更後 |
|---|---|---|
| First Contentful Paint | 480ms | **276ms** |
| 描画をせき止めた通信 | 1件 **149,255バイト** | **0件 0バイト** |

ローカル回線での数値なので、電波の弱い現場ではこの差はさらに大きくなる。

**さらに速くしたい場合（未実施）**：Webフォントを完全にやめて端末標準フォントだけにすると、フォント通信が丸ごと0になる（数百KB〜1MBの削減）。ただしロゴ・見出しの丸ゴシック（Zen Kaku Gothic New）の雰囲気は変わる。要望があれば切り替える。

検証：全23シーン エラー0件。

### 2026-09-21: スマホのヘッダーで時計を右上に移動（v103）
- 620px以下の`.hdr`のグリッドを `"mascot logo sync" / "center center status"` から
  **`"mascot logo center sync" / "status status status status"`** に変更し、`.hdr-clock`を`justify-self:end`にした
- 1段目が「マスコット → ロゴ → 時計 → 同期ボタン」、2段目がステータスのピルだけになる
- **ヘッダーの高さは 118px → 102px に縮んだ**（2段目が軽くなったため）。固定バーの合計高さが減るので、iPhoneで本文に使える領域が増える
- 実測（375/390/430px）：時計は1段目・右端から55px（同期ボタンの左隣）、横はみ出しなし

検証：全23シーン エラー0件。

### 2026-09-23: 待ち時間バーを廃止し、待ち目安を時計の横へ（v104・フロントのみ）
- `.wbar`要素を削除し、`#wbar-num`・`#wdots`はヘッダー内の`.hdr-center > .hdr-wait`へ移動（idは据え置きなので`render`側の更新処理は無変更）
- `updateStickyOffsets()`は`.wbar`が無ければ高さ0として計算する（`--wbar-top`は未使用になった）
- 時計のグリッド配置（`grid-area:center`）は`.hdr-clock`ではなく`.hdr-center`が持つ。スマホの右寄せ（`justify-self:end`）も`.hdr-center`側
- 1段目に5要素並ぶため、440px以下でロゴ縮小、370px以下でマスコット非表示。実測（360/375/390/430/620/768/1024/1400px）でロゴと時計の重なり・横はみ出しなし、ヘッダーとタブのすき間0
- **この作業はMac機で行った**。Mac側 `~/Desktop/開発/進捗管理・売上管理` は `.git` ありで v95 のまま止まっていたので、`git fetch`→`merge --ff-only` で v103 に追従させてから作業した（ドライブ側＝リモートと一致を確認済み）

### 2026-09-23: ヘッダーの文字バランス統一（v105・フロントのみ）
- 「タイトルと時計、目安時間の文字バランスが悪い」との指摘。タイトル・時計・待ち目安の文字サイズが各所でばらばらに指定されていた
- `/* ---- ヘッダーの文字サイズ統一（v105） ---- */` のブロックに集約。`.hdr`に `--hs`（小見出し）`--hn`（数字の箱）`--hl`（タイトル）`--hpad` `--hr` を置き、1000/620/440px のメディアクエリでは**この変数だけ**を変える。**ヘッダー内の文字サイズを変えるときは個別に`font-size`を書かず、変数を変えること**（古いルールが`!important`で残っているため、個別指定はこのブロックに負ける）
...
### 2026-09-23: 「◯分待ち」を完成時刻の横に大きく（v108・フロントのみ）
- `#wait-min`を見出し行から`.eta-row`（`#wbar-num`の右隣）へ移し、`<b>N</b>分待ち`で出す。サイズは`--hn`基準（数字1.05倍・「分待ち」0.5倍）
- 820px以下は`.hdr-mascot`を非表示（グリッドの1列目を0幅に）。400px以下は`--hn:16px`
- 320px（初代iPhone SE）では「120分待ち」で時計がタイトルに約30px重なる。対応端末として考えていないので未対応
